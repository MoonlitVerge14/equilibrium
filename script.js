...
// NOTE: window.supabase is the CDN library — we name our client 'sb' to avoid conflict
const sb = window.supabase.createClient('https://zylysujwseykewcrvcfr.supabase.co', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inp5bHlzdWp3c2V5a2V3Y3J2Y2ZyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY2OTUzMDEsImV4cCI6MjA5MjI3MTMwMX0.kRMRpgmxsJFBIDO8MShrC_TtKJKfj2Z6iDuFAAczWV4');
...
const stories = [
  ...
  },
  {
    ...
    link: 'https://www.kennedy-center.org/artists/w/wa-wn/oprah-winfrey/',
  ...
  },
  {
    ...
    link: 'https://www.youtube.com/watch?v=UibfDUPJAEU',
  ...
  },
  {
    ...
    link: 'https://english.ckgsb.edu.cn/story/jack-ma-leader-of-the-e-commerce-revolution-in-china/',
    linkText: 'Read about Jack Ma →'
  },
  {
    ...
    link: 'https://www.businessinsider.com/jack-dorsey-twitter-creator-billionaire#he-got-a-job-at-a-podcasting-company-named-odeo-where-he-met-his-future-twitter-cofounders-3',
    linkText: 'Read the full Business Insider article →'
  }
];
...
async function toggleLike(postId) {
  ...
  if (existing) {
    ...
    await sb.from('posts').update({ likes: Math.max(0, post.likes - 1) }).eq('id', postId);
    post.likes = Math.max(0, post.likes - 1);
    post._liked = false;
  } else {
    // Like
    await sb.from('likes').insert({ user_id: currentProfile.id, post_id: postId });
    await sb.from('posts').update({ likes: post.likes + 1 }).eq('id', postId);
...
async function loadAndUpdateStreak() {
  ...
  if (error || !data) {
    // Insert if missing
    await sb.from('streaks').insert({ user_id: currentProfile.id, streak: 1, last_visit: today });
